//! BentoScript presets database.

//# feather use syntax-errors

/// Represents the set of environment presets understood by __BentoScript.
/// When used with [setPreset], this enum determines what functions,
/// constants, and keywords get exposed.
enum __BENTOSCRIPT_PRESET {
...
/// @return {Function}
function __BentoScriptPresetGet(preset) {
    static _global = __BentoScriptGMLGlobal();
    var presetFunc = _global.__bentoScriptPresets[$ preset];
    if (BENTOSCRIPT_DEBUG_MODE && presetFunc == undefined) {
        __BentoScriptError(
            "a BentoScript preset with the key '",
            preset, "' does not exist, make sure the preset exists in the ",
            "`__BENTOSCRIPT_PRESET` enum"
        );
    }
    return presetFunc;
}
...
/// @ignore
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetType(ffi) {
    ffi.exposeNativeFunction(
        "is_string", is_string,
        "is_real", is_real,
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetArray(ffi) {
    ffi.exposeNativeFunction(
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetStruct(ffi) {
    ffi.exposeNativeFunction(
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetString(ffi) {
    ffi.exposeNativeFunction(
        "ansi_char", ansi_char,
        "chr", chr,
        "ord", ord,
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetMaths(ffi) {
    ffi.exposeNativeFunction(
        "round", round,
        "frac", frac,
        "abs", abs,
    ...
    ffi.exposeNativeConstant("pi", pi);
}

/// @ignore
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPreset3DMaths(ffi) {
    ffi.exposeNativeFunction(
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetColour(ffi) {
    ffi.exposeNativeFunction(
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetDraw(ffi) {
    ffi.exposeNativeFunction(
        "draw_self", draw_self,
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetRandom(ffi) {
    ffi.exposeNativeFunction(
        "choose", choose,
        "random", random,
...
///
/// @param {Struct.__BentoScriptClassForeignInterface} ffi
function __BentoScriptPresetUnsafe(ffi) {
    ffi.exposeNativeFunction(
...
///   add an `rgb` function to the given [__BentoScriptClassEnvironment].
///
/// ```gml
/// __BentoScriptPresetAdd("my-custom", function (ffi, keywords) {
///   ffi.exposeNativeFunction("rgb", make_colour_rgb);
/// });
/// ```
function __BentoScriptPresetAdd(key, callback) {
    static _global = __BentoScriptGMLGlobal();
    if (BENTOSCRIPT_DEBUG_MODE) {
        __BentoScriptCheckInit();
    }
    var presets = _global.__bentoScriptPresets;
    if (variable_struct_exists(presets, key)) {
       __BentoScriptError("a preset with the key '", key, "' already exists");
    }
    presets[$ key] = callback;
}

/// @ignore
function __BentoScriptInitPresets() {
    static _global = __BentoScriptGMLGlobal();
    /// @ignore
    _global.__bentoScriptPresets = {};
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.TYPE, __BentoScriptPresetType);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.ARRAY, __BentoScriptPresetArray);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.STRUCT, __BentoScriptPresetStruct);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.STRING, __BentoScriptPresetString);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.MATH, __BentoScriptPresetMaths);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.MATH_3D, __BentoScriptPreset3DMaths);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.COLOUR, __BentoScriptPresetColour);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.DRAW, __BentoScriptPresetDraw);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.RANDOM, __BentoScriptPresetRandom);
    __BentoScriptPresetAdd(__BENTOSCRIPT_PRESET.UNSAFE, __BentoScriptPresetUnsafe);
}