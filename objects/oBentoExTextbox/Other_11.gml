...
draw_sprite_ext(sprite_index, image_index, x + _offset, y + _offset, image_xscale, image_yscale, image_angle, BentoTextGetTargeted()? BENTO_EXAMPLE_RED : image_blend, image_alpha);

if (BentoTextGetTargeted())
{
    var _text = __displayText;
    if ((BentoTextGetOpenDuration() mod 30) < 15) _text += "|";
    draw_set_color(BENTO_EXAMPLE_RED);
}
else
{
    if (__displayText == "")
