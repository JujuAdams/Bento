    ...
    repeat(array_length(_stepOrder))
    {
        ...
        else if (_methodFunc == __GuiScissorPop)
        {
            ...
            with(method_get_self(_stepOrder[_i]))
            {
                ...
                _scissorL = max(_scissorL, bbox_left   + GUI_STRUCT.__scissorPadLeft  );
                _scissorT = max(_scissorT, bbox_top    + GUI_STRUCT.__scissorPadTop   );
                _scissorR = min(_scissorR, bbox_right  - GUI_STRUCT.__scissorPadRight );
                _scissorB = min(_scissorB, bbox_bottom - GUI_STRUCT.__scissorPadBottom);
            }
