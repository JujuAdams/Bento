...
function __BentoSolverTextGetShrinkHeight()
{
    ...
    if (__layoutHeightResize == BENTO_RESIZE_SHRINK)
    {
        ...
        var _height = clamp(__funcMeasureHeight(__attachedElement.text, -1, __solvedWidth), __layoutHeightMin, __layoutHeightMax);
        __solverMinHeight = _height;
        __solvedHeight    = _height;
    }
    else
    {
        __solverMinHeight = (__layoutHeightMin > 0)? __layoutHeightMin : __funcMeasureHeight(__attachedElement.text, 999999);
