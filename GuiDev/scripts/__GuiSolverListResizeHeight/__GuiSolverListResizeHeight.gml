...
function __GuiSolverListResizeHeight()
{
    ...
    else
    {
        ...
        if (_remaining > 0)
        {
            ...
            repeat(_childCount)
            {
                ...
                if (_child.GUI_STRUCT.__layoutHeightResize == GUI_RESIZE_GROW)
                {
            ...
            while (_remaining > 0)
            {
                ...
                repeat(array_length(_modifiableArray))
                {
                    ...
                    var _childSize = _child.GUI_STRUCT.__solvedHeight;
                    if (_childSize < _min)
                    {
                        _secondMin = _min;
                        _min = _childSize;
                ...
                repeat(_workCount)
                {
                    ...
                    var _prevSize = _child.GUI_STRUCT.__solvedHeight;
                    if (_prevSize == _min)
                    {
                        ...
                        _child.GUI_STRUCT.__solvedHeight = _newSize;
        ...
        else if (_remaining < 0)
        {
            ...
            {
                var _child = _childArray[_i];
                if (_child.GUI_STRUCT.__layoutHeightResize != GUI_RESIZE_STATIC)
                {
                    array_push(_modifiableArray, _child);
                }
            ...
            while ((_remaining < 0) && (array_length(_modifiableArray) > 0))
            {
                ...
                repeat(_workCount)
                {
                    ...
                    if (_prevSize == _max)
                    {
                        ...
                        {
                            var _newSize = _child.__layoutHeightMin;
                            array_delete(_modifiableArray, array_get_index(_modifiableArray, _child), 1);
                        }
                        else
                        {
