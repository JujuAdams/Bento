...
function __GuiSolverListResizeWidth()
{
    ...
    var _remaining = layoutWidth - __solverFitWidth;
    ...
    if (__listAxis == GUI_AXIS_Y)
    {
        ...
        var _available = layoutWidth - (__layoutPadLeft + __layoutPadRight);
        if (_available > 0)
        {
            var _i = 0;
            repeat(_childCount)
            {
                with( _childArray[_i])
                {
                    ...
                    {
                        layoutWidth = clamp(min(layoutWidth, _available), __layoutWidthMin, __layoutWidthMax);
                    }
                    else if (__layoutWidthResize == GUI_RESIZE_GROW)
                    {
                        layoutWidth = clamp(_available, __layoutWidthMin, __layoutWidthMax);
                    }
                }
    ...
    else
    {
        ...
        if (_remaining > 0)
        {
            ...
            repeat(_childCount)
            {
                ...
                if (_child.__layoutWidthResize == GUI_RESIZE_GROW)
                {
            ...
            while ((_remaining > 0) && (array_length(_modifiableArray) > 0))
            {
                ...
                repeat(array_length(_modifiableArray))
                {
                    ...
                    var _childSize = _child.layoutWidth;
                    if (_childSize < _min)
                    {
                        _secondMin = _min;
                        _min = _childSize;
        ...
        else if (_remaining < 0)
        {
            ...
            {
                var _child = _childArray[_i];
                if (_child.__layoutWidthResize != GUI_RESIZE_STATIC)
                {
                    array_push(_modifiableArray, _child);
                }
