...
|-----------------|--------------------|--------------------------------------------------------------------------------------------|
| callbackStep    | CallbackSetStep    | Called every frame via UI_ONION.Step()                                                     |
| callbackDraw    | CallbackSetDraw    | Called every frame via UI_ONION.Draw()                                                     |
...
| callbackRelease        | CallbackSetButtonEnd        | Function to call when a button is released (and the UI element received a pressed event)                                                               |
|                        |                             | N.B. This is NOT the same as "clicking" the button. A button can be released for many reasons that aren't the same as the player deliberately clicking |
