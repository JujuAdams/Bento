...
|"hover"   |Both ❌ and ✅              |✅                          |`BentoCursorGetHover()`                                                   |
...
|"hold"    |Both ❌ and ✅           |✅                      |`BentoPrimaryGetHold()`    |
...
A non-obvious but significant use case for focus is to create completely custom element types. This might include a scrollable map or a rotary dial. An element that is focused can still receive input from elsewhere, including functions such as `BentoCursorGetDX()` and `BentoCursorGetDY()`. There are many possibilities when input is restricted to one element using Bento's focus feature.
